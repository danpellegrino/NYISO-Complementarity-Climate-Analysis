...
seasonal_energy_demand <- function(season) {
  ...
  # Smooth the data
  demand_data$value <- smooth.spline(demand_data$value, spar = 0.2)$y

  # Plot the energy demand across the seasons ontop of each other
  ggplot() +
    geom_line(data = demand_data, aes(x = hour, y = value), color = "blue") +
    labs(
      title = paste("Energy Demand Across the ", detect_season(season), " Season", sep = ""),
      x = "Hour",
      y = "Energy Demand (MW)"
    ) +
